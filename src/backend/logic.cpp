...
static bool check_cycle_recursive(Block* current, Block* target) {
    if (!current) return false;
    if (current == target) return true;
    
    if (check_cycle_recursive(current->next, target)) return true;
    if (check_cycle_recursive(current->inner, target)) return true;
...
bool would_create_cycle(Block* top, Block* bottom) {
    ...
    return check_cycle_recursive(bottom, top);
}
